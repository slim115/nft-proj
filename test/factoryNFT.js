...
describe("Minting the token and returning it", function () {
  it("should the contract be able to mint a function and return it", async function () {
    ...
    const FactoryContract = await ethers.getContractFactory("MyToken"); // Getting the contract
    ...
    const tokenURI = await factoryContract.tokenURI(tokenId) // Using the tokenURI from ERC721 to retrieve the metadata
